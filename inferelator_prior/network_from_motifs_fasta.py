from inferelator_prior.network_from_motifs import (
    add_common_arguments,
    parse_common_arguments,
    load_and_process_motifs,
    scan_and_build_by_tf,
)
...
def main():
    ...
    _, _, prior_data = build_motif_prior_from_fasta(
        args.motif,
        args.fasta,
...
def build_motif_prior_from_fasta(
    motif_file,
    promoter_fasta_file,
    scanner_type="fimo",
    num_cores=1,
    ...
    """
    Build a motif-based prior from sequences extracted into a FASTA.

    :param motif_file: Path to motif file (meme or transfac format)
    :type motif_file: str
    :param promoter_fasta_file: Path to FASTA file containing sequence DNA
    ...
    :param motif_ic: Minimum informational content required to keep a motif
        hit. Defaults to 6 bits
    :type motif_ic: numeric
    :param tandem: Maximum distance to consider two TF bindings a single
        tandem array
    :type tandem: int
    :param truncate_prob: Truncate probabilities that flank a PWM/PFM until
        this value is reached. None disables.
    ...
    :param gene_constraint_list: A file that limits modeling to a
    ...
    :param motif_info: Information about motifs. This will override any
        details read in from motif file. A template can be generated from
        ``inferelator_prior.motif_information`` and modified as needed.
    :type motif_info: pd.DataFrame
    :param shuffle: Randomly shuffle motif PWMs using this seed. None disables.
        Defaults to None.
    ...
    print(
        f"Scanning promoter sequences ({promoter_fasta_file}) "
    ...
    return scan_and_build_by_tf(
        ...
        num_cores=num_cores,
        extract_genome=False,
        filter_motif_hits_for_gene_list=gene_constraint_list is not None,
        by_chromosome=False,
    )
